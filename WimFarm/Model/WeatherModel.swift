...
//  WimFarm
//
//  Created by ulalalab on 2022/03/29.
//
...
struct WeatherModel {
  ...
  static func convert(fromResponse response: OneCallResponse) -> WeatherModel {
      WeatherModel(latitude: response.lat,
...
struct WeatherCurrentModel {
    ...
    static func convert(fromResponse response: CurrentResponse) -> WeatherCurrentModel {
        WeatherCurrentModel(time: Date(timeIntervalSince1970: TimeInterval(response.dt)),
...
struct WeatherHourlyModel { //  Mappable {
    let time: Date
    let actualTemp, feelsLikeTemp: Temperature
    let pressure, humidity: Int
    ...
    static func convert(fromResponse response: HourlyResponse) -> WeatherHourlyModel {
        WeatherHourlyModel(time: Date(timeIntervalSince1970: TimeInterval(response.dt)),
                           actualTemp: .init(kelvin: response.temp),
                           feelsLikeTemp: .init(kelvin: response.feelsLike),
                           pressure: response.pressure,
                           humidity: response.humidity,
...
struct WeatherDailyModel { //  Mappable {
    let time, sunriseTime, sunsetTime: Date
    ...
    static func convert(fromResponse response: DailyResponse) -> WeatherDailyModel {
        WeatherDailyModel(time: Date(timeIntervalSince1970: TimeInterval(response.dt)),
                          sunriseTime: Date(timeIntervalSince1970: TimeInterval(response.sunrise)),
                          sunsetTime: Date(timeIntervalSince1970: TimeInterval(response.sunset)),
